...
#+sbcl
(test cached-numeric-module-load-allocation
  ;; Loading cached numeric code must not repeat hundreds of MB of table work.
  (let* ((component (asdf:find-component (asdf:find-system "com.inuoe.jzon")
                                        '("src" "eisel-lemire")))
         (fasl (first (asdf:output-files 'asdf:compile-op component)))
         (before (sb-ext:get-bytes-consed)))
    (load fasl :verbose nil :print nil)
    (is (< (- (sb-ext:get-bytes-consed) before) (* 16 1024 1024)))))
