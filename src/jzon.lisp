...
(macrolet ((def-make-ub8-fns (name type)
             `(defun ,name (in start end max-string-length)
               ...
               (let ((i start))
                 (declare (type (integer 0 #.array-dimension-limit) i))
                 (let* ((n 1)
                        (consume-octet (lambda () (if (< i end) (aref in (shiftf i (1+ i))) (error "End of data while in UTF-8 sequence."))))
                        (step (lambda () (when (< i end) (incf n) (%utf-8-decode (aref in (shiftf i (1+ i))) consume-octet))))
                        (pos (lambda () (setf i start) (%calc-pos step n)))
                        (read-string (let ((string-accum (make-array (min 256 (1- array-dimension-limit)) :element-type 'character :adjustable t :fill-pointer 0)))
                                       (lambda ()
...
(defun %make-fns-binary-stream (in max-string-length)
  ...
  (declare (type (integer 1 (#.array-dimension-limit)) max-string-length))
  (let* ((start (ignore-errors (file-position in)))
         (n 1)
         (consume-octet (lambda () (or (read-byte in nil nil)
                                       (error "End of data while in UTF-8 sequence."))))
         (step (lambda () (let ((u1 (read-byte in nil nil))) (when u1 (incf n) (%utf-8-decode u1 consume-octet)))))
         (pos (lambda ()
                (let ((bpos (ignore-errors (file-position in))))
                  (if (and bpos (ignore-errors (file-position in start)))
                    (multiple-value-bind (line col) (%calc-pos step n)
                      (file-position in bpos)
                      (values line col))
                    (values nil nil)))))
         (read-string (let ((string-accum (make-array (min 256 (1- array-dimension-limit)) :element-type 'character :adjustable t :fill-pointer 0)))
                        (lambda ()
...
(defun %make-fns-stream (in max-string-length)
  ...
  (let* ((start (ignore-errors (file-position in)))
         ...
         (pos (lambda ()
                (let ((n (and start (ignore-errors (file-position in)))))
                  (if (and n (ignore-errors (file-position in start)))
                    (multiple-value-bind (line col) (%calc-pos step n)
                      (file-position in n)
                      (values line col))
                    (values nil nil)))))
