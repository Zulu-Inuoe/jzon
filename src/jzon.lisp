    ...
    (with-output-to-string (s)
      (macrolet ((#1=#:|| ()
                  `(etypecase key
                    ...
                    ,@(unless (multiple-value-bind (x1 y1) (subtypep 'short-float 'single-float)
                                (multiple-value-bind (x2 y2) (subtypep 'single-float 'short-float)
                                  (and x1 y1 x2 y2)))
                      '((short-float  (sf:write-float (coerce key 'single-float) s))))
                    ,@(unless (multiple-value-bind (x1 y1) (subtypep 'long-float 'long-float)
                                (multiple-value-bind (x2 y2) (subtypep 'long-float 'long-float)
                                  (and x1 y1 x2 y2)))
